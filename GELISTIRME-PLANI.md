...
- [x] **E6.3** Yön hatası tespiti: çizim yönü kılavuzun tersineyse "yukarıdan aşağı gidelim" gibi uyarı. `pathParamPoints()` kılavuz vuruşunu 0-1 arası kümülatif uzunluk parametresine (`t`) çevirir; `strokeDirectionOk(userPts,strokePath,size)` çocuğun çizdiği noktaların ilk %25'i ile son %25'inin en yakın `t` ortalamasını karşılaştırır (son, baştan belirgin küçükse ters yön). Çok vuruşlu harflerde `end()` içinde uzamsal örtüşme (`strokeMatchRatio`) yeterliyken yön tersse, vuruşu ilerletmeden "Yönü ters çevirmişsin, yeşil noktadan başlayıp okun gösterdiği yöne gidelim" nazik uyarısı gösterilir (engellemez, tekrar denenebilir). — 2026-09-04
- [x] **E6.4** "Havada çiz" ısınması (ders 0 zaten var) + parmak kası ısınma çizgi çalışmaları (dalga, zikzak, spiral). Ders 0'ın serbest çizim adımından (`stepHazirlik`, zaten vardı) sonra 3 yeni kılavuzlu ısınma adımı eklendi (`buildSteps` tip:'hazirlik' artık 4 adım döndürüyor) — `WARMUP_SHAPES` (dalga/zikzak/sarmal, sarmal `spiralPoints()` ile üretiliyor) + `stepHazirlikSekil()`/`setupWarmupTrace()`/`checkWarmupTrace()` (harf maskesine değil `strokeMatchRatio` ile kılavuz çizgisine örtüşmeye göre değerlendiriyor, engellemez — istenildiği kadar tekrar). `runStep()` yeni `hazirlikSekil` oyun tipini yönlendiriyor. — 2026-09-04
- [x] **E6.5** Büyük harf çizimi (ayrı `STROKES_UPPER`). Türk alfabesinin 29 harfinin BÜYÜK hâli için `STROKES_UPPER` (aynı küçük-harf anahtarları, `UPPER_MAP` ile aynı glyph eşlemesi). `setupTrace(letter,level,upper)` üçüncü parametre alır — `upper` true iken `STROKES_UPPER` + BÜYÜK glyph kullanır, `state.soundStats[letter]` anahtarı aynı kalır ama ayrı `cizLevelUpper` alanında ilerler (küçük harf `cizLevel`'ı etkilemez). Yeni `roundCizBuyuk()` + Serbest Oyun menüsüne "🔠 Büyük Harf Çiz" kartı (yalnız Çözümleme modunda, `freeBuyuk`/`freeOkuma` deseniyle aynı). — 2026-09-04
...
- 2026-09-04 — E6.3 (rutin, backlog sırası): Harf Çiz'e yön hatası tespiti eklendi — `pathParamPoints()` kılavuz vuruşunu kümülatif uzunluğa göre 0-1 parametreye çevirir, `strokeDirectionOk()` çizilen noktaların ilk/son çeyreğinin ortalama parametresini karşılaştırıp tersten çizilip çizilmediğini saf/test edilebilir bir fonksiyonla belirler. Çok vuruşlu harflerde uzamsal örtüşme yeterli ama yön tersse (`end()` içinde), vuruş ilerletilmeden nazik bir yön uyarısı gösteriliyor — mevcut "biraz farklı deneyelim" uyarısından ayrı, daha spesifik bir mesaj; hiçbir durumda engellemiyor, çocuk istediği kadar tekrar deneyebiliyor. Smoke test'e 4 yeni kontrol (342 test toplamda). Sıradaki öncelik: E6.4 (havada çiz ısınması) / E6.5 (büyük harf çizimi) veya E5.4 (erişilebilirlik geçişi tam taraması).

- 2026-09-04 — E6.5 (rutin, backlog sırası): büyük harf çizimi eklendi — Türk alfabesinin 29 harfinin BÜYÜK hâli için `STROKES_UPPER` (aynı anahtarlar, gerçek yazım yönünde vuruşlar: yukarıdan aşağı, soldan sağa). `setupTrace()` yeni `upper` parametresiyle `STROKES_UPPER`+BÜYÜK glyph'e geçiyor; ilerleme küçük harften bağımsız ayrı `cizLevelUpper` alanında tutuluyor (`cizLevelUpper()` yardımcı fonksiyonu, E6.2'deki `cizLevel()` deseniyle aynı). Yeni `roundCizBuyuk()` + Serbest Oyun menüsüne yalnız Çözümleme modunda görünen "🔠 Büyük Harf Çiz" kartı. Smoke test'e 11 yeni kontrol (353 test toplamda). **E6 epiği neredeyse tamamlandı** — yalnız E6.4 (havada çiz ısınması) kaldı. Sıradaki öncelik: E6.4 veya E5.4 (erişilebilirlik geçişi tam taraması) / E7 (PWA & sağlamlık).

- 2026-09-04 — E6.4 (rutin, backlog sırası — **E6 epiği tamamlandı**): ders 0'ın (Hazırlık) mevcut serbest çizim adımından sonra 3 kılavuzlu parmak kası ısınma çizgisi eklendi — dalga, zikzak, sarmal (`WARMUP_SHAPES`, sarmal `spiralPoints()` ile parametrik üretiliyor). Harf maskesi kullanmıyor; `checkWarmupTrace()` çizilen noktaların kılavuz çizgisine `strokeMatchRatio` ile örtüşme oranına bakıyor, yetersizse nazikçe tekrar dener (engellemez). `buildSteps()` tip:'hazirlik' artık 4 adım döndürüyor, `runStep()` yeni `hazirlikSekil` oyun tipini `stepHazirlikSekil()`'e yönlendiriyor. **Böylece E6 epiği (Harf Çiz derinleştirme: vuruş kılavuzu, 5 kademe, yön hatası, ısınma çizgileri, büyük harf) tamamlandı.** Smoke test'e 7 yeni kontrol (360 test toplamda). Sıradaki öncelik: E5.4 (erişilebilirlik geçişi tam taraması) veya E7 (PWA & sağlamlık).
