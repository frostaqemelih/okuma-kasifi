...
- [ ] **E4.4** 5. grup `p,h,v,ğ,f,j`: benzer. (ğ kelime başında olmaz — kural göster.)
  - [x] (a)-(c) **p, h, v** tam donanımlı eklendi: `mufredat.json` sesler.p/h/v + ders26/27/28, `index.html`'e `WORDS.p` (para💵), `WORDS.h` (horoz🐓 — "hava" değil, çünkü henüz öğrenilmemiş 'v' sesini içeriyordu), `WORDS.v` (vazo🏺), `STROKES.p` (2 vuruş: dik gövde+üst ilmek), `STROKES.h` (2 vuruş: uzun gövde+kemer, n'nin deseniyle aynı), `STROKES.v` (1 vuruş: V şekli), `LESSONS` id26-28, `ALL_LETTERS`/`UPPER_MAP`'e h/v eklendi, `WORDBANK`'a para💵/horoz🐓/vazo🏺. Kalan sesler (ğ — kelime başında/sonunda hiç olmaz, ayrı özel-durum mühendisliği gerektiriyor — f, j) sonraki çalışmalara bırakıldı. — 2026-09-04
